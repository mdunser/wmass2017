...
def harvestEm(subdir, mwrange, charge='both'):
    ...
    for card in glob.glob(subdir+'/dc_mass*.txt'): #.format(ch='' if charge == 'both' else 'plus*' if charge == 'plus' else 'minus*')):
        # old configuration cmb.QuickParseDatacard(card, """dc_mt_mass(?<MASS>\d+)_$CHANNEL.txt""")
        cmb.QuickParseDatacard(card, """dc_mass(?<MASS>\d+)_$CHANNEL.txt""")
    ...
    cmb.FilterObs(lambda obj: obj.mass() != '96')
    ...
    for isub, subdir in enumerate(subdirs):
        ...
        if runHarvest: 
            ...
            if charge == 'both':
                ...
                target_dc = '{subdir}/w_cards_morphed_{charge}/morphed_datacard_channel.txt'.format(subdir=subdir,charge=charge)
                if os.path.isfile(target_dc):
                    print 'removing existing combined datacard first!'
                    os.system('rm {dc}'.format(dc=target_dc) )
            #elif charge == 'plus':
            #    dcs = '{subdir}/w_cards_morphed_{charge}/plus*.txt  '.format(subdir=subdir,charge=charge)
            #    target_dc = '{subdir}/w_cards_morphed_{charge}/morphed_datacard_plus.txt'.format(subdir=subdir,charge=charge)
            #elif charge == 'minus':
            #    dcs = '{subdir}/w_cards_morphed_{charge}/minus*.txt '.format(subdir=subdir,charge=charge)
            #    target_dc = '{subdir}/w_cards_morphed_{charge}/morphed_datacard_minus.txt'.format(subdir=subdir,charge=charge)
        ...
        impactBase += ' --setPhysicsModelParameters mw=101,r=1  --redefineSignalPOIs=mw --setPhysicsModelParameterRanges mw={mwrange} -t -1 '.format(mwrange=mwrange)
